...
/* Restored BARAMEEL RUN arcade sound language from the earlier build. */
...
function rewardSound(){
  ...
  coins.forEach((f,i)=>{tone(f,.085,'square',.76,i*.065,i<coins.length-1?90:180);tone(f*2,.055,'triangle',.22,i*.065+.012,120)});
  [784,988,1175,1568,2093].forEach((f,i)=>tone(f,.13,'square',.64,.48+i*.055,i===4?240:70));
  tone(1047,.38,'triangle',.34,.78,180);tone(1568,.44,'triangle',.30,.82,260);noise(.13,.18,.02);noise(.08,.12,.56);
}
function play(type){
  if(type==='tap'||type==='select'){selectSound(selected());return;}
  if(type==='confirm'){confirmSound();return;}
  if(type==='success'){rewardSound();return;}
  if(type==='scan'){scanSound();return;}
  if(type==='error'){errorSound();return;}
  if(type==='back'){backSound();return;}
  tone(520,.10,'sine',.08);
}
...
function parseQR(raw){
  ...
  return {collection:collectionMatch?`collection${String(collectionMatch[1]).padStart(2,'0')}`:'collection01',piece:Number(pieceMatch[1])};
}
function startRewardTransition(piece){
  const now=Date.now();
  localStorage.setItem('barameelRewardTransition',JSON.stringify({startedAt:now,piece:Number(piece)||0}));
  playRewardFrom(0);
}
document.addEventListener('pointerdown',()=>{audio();},{once:true,passive:true});
window.BR={RUNNERS,RUNNER_NAMES,state,save,setRunner,setNickname,addPoints,collect,selected,count,hasPiece,play,selectSound,confirmSound,scanSound,errorSound,backSound,rewardSound,rewardAudio,playRewardFrom,pulse,flash,go,preload,preloadAll,setArt,parseQR,startRewardTransition};
